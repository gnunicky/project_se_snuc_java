...
1. Come prima operazione bisogna scaricare il software con i relativi sorgenti e l'eseguibile utilizzando:
   git clone https://github.com/gnunicky/project_se_snuc_java.git
    oppure
   wget https://github.com/gnunicky/project_se_snuc_java/archive/master.zip
2. Sono presenti le seguenti cartelle:
   ...
   Inolte nella cartella principale sono presenti i due applicativi SnucClient.jar e il SnucServer.jar.
3. Per avviare gli applicativi, senza utilizzare un ambiente di sviluppo IDE, lasciando la posizione dei file invariati, basta digitare da terminale:
   - java -jar SnucClient.jar per il client;
   - java -jar SnucServer.jar per il server.
4. Per chi visualizza il codice sorgente, bisogna avere installato Netbeans (IDE), clonando o importando i vari repository
...
N.B.
REQUISITI DEL CLIENT: per un corretto avvio del client è necessario inserire nella cartella ../config il file di testo chiamato Config.txt.
- Nel file è presente il tipo di comunicazione utilizzata. Come prima riga del file occorre inserire: TCP o UDP. Naturalmente il server dovrà
  comunicare con lo stesso tipo di protocollo del client. 
...
- Nel primo file saranno presenti la lista delle stanze caricate dall'amministratore del sistema. Secondo la medesima sintassi:
  ...
  [...]
  #RoomN 
- Nel secondo file è presente il tipo di comunicazione utilizzata. Come prima riga del file occorre inserire: TCP o UDP. Naturalmente il client dovrà
